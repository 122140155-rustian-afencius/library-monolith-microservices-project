...
// Update book quantity (for internal service communication)
exports.updateBookQuantity = async (req, res) => {
  try {
    ...
    book.availableQuantity += increment;

    // Make sure it doesn't go below 0 or above total quantity
    book.availableQuantity = Math.max(
      0,
      Math.min(book.quantity, book.availableQuantity)
    );
